...
plot_chart2 <- function(lattes.list,
  ...
  myPubsQualis          <- as.data.frame(
                              with(lattes.list$`Journal Papers`,table(Year,Qualis)))
  # myPubsQualis          <- do.call(rbind.data.frame, myPubsQualis)



  # myPubsQualis$Qualis   <- gsub("\\..*","", rownames(Qualis))
  # names(myPubsQualis)   <- c("Year", "Count", "Type")
  myPubsQualis$Year     <- as.numeric(as.character(myPubsQualis$Year))
  myPubsQualis          <- myPubsQualis[order(myPubsQualis$Year), ]
  # rownames(myPubsQualis)<- paste0(myPubsQualis$Year) # duplicate 'row.names' are not allowed

  # Second data frame containing all entries by year (even if with 0 occurrences)
  # myPubsQualis2           <- cbind(expand.grid(Year   = unique(myPubsQualis$Year),
  #                                            Qualis = unique(myPubsQualis$Qualis)),
  #                             Count = 0)
  # rownames(myPubsQualis2) <- paste0(myPubsQualis2$Year)
  #
  # # Populate counts for myTotals2
  # for (i in 1:nrow(myPubsQualis)){
  #   indx <- which(rownames(myPubsQualis2) == rownames(myTotals)[i])
  #   if(length(indx)){
  #     myPubsQualis2$Freq[indx] <- myPubsQualis$Freq[i]
  #   }
  # }
  ...
  if (type == "ggplot2" | type == "plotly"){
    if (language == "PT"){
      ...
      my.ylab <- "Quantidade"
      my.legtitle <- "Qualis"
      my.title <- "Número de Publicações - Qualis (2013-2016)"
    }
    if (language == "EN"){
      my.xlab <- "Year"
      my.ylab <- "Count"
      my.legtitle <- "Qualis"
      my.title <- "Publication Count - Qualis (2013-2016)"
    }
    ...
    myPlot2 <- ggplot2::ggplot(data    = myPubsQualis,
                              mapping = ggplot2::aes(x    = myPubsQualis$Year,
                                                     y    = myPubsQualis$Freq,
                                                     fill = myPubsQualis$Qualis)) +
      ggplot2::geom_bar(stat     = "identity",
                        position = "stack",
                        colour   = "#00000011") +
      ggplot2::ggtitle(my.title) +
      ggplot2::scale_x_continuous(breaks = min(myPubsQualis$Year):max(myPubsQualis$Year)) +
      ggplot2::theme(axis.text.x     = ggplot2::element_text(angle = 45, vjust = 0),
                     legend.position = "bottom") +
      ggplot2::xlab(my.xlab) +
      ggplot2::ylab(my.ylab) +
    ...
    myPlot3 <- ggplot2::ggplot(data    = myPubsQualis,
                               ggplot2::aes(x    = Year,
                                            y    = Freq,
                                            color = Qualis)) +
      ggplot2::geom_line() +
    ...
    if (type == "plotly"){
      myPlot2 <- plotly::ggplotly(myPlot2,
                                 width  = width,
                                 height = height)
    }
    return(myPlot2)
