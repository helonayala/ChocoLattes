...
set_qualis2 <- function(lattes.list, qualis.file, which.qualis){
  ...
  # how many papers to iterate?
  npapers = dim(lattes.list$`Journal Papers`)[1]

  # init fields
  lattes.list$`Journal Papers`$Qualis = "NA"
  ...
  if(npapers){
    for (j in 1:npapers){
      ...
      mydoc <- lattes.list$`Journal Papers`[j,]
      mydoc <- setqualis2(x = mydoc,
                          qualis.df = qualis,
                          which.qualis = which.qualis)

      if(mydoc$ValidQualis)
        lattes.list$`Journal Papers`[j,]$Qualis = mydoc$Qualis
    }
  }
  return(lattes.list)
}
...
setqualis2 <- function(x, qualis.df, which.qualis){
